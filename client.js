...
on("__cfx_nui:GiveItem", (data, cb) => {
	...
	emit("hud-display-item",id,"Added",amount)
...
on('inventory:removeItem', (id, amount) => {
	...
	emit("hud-display-item",id,"Deleted", amount)
})
...
on('__cfx_nui:dropIncorrectItems', (data, cb) => {
	...
	canOpen = false;
	emitNet("server-inventory-open", GetEntityCoords(PlayerPedId()), cid, "13", "Ground", data.slots);
...
RegisterNuiCallbackType("inventory:weaponSwap");
on("__cfx_nui:inventory:weaponSwap", (data, cb) => {
